...
## S9：D3-light 五任务面板冻结（2026-08-11）

在 Git 基线冻结后，第一章没有直接重跑全部 20 条，而是按运行机制覆盖从原始问题中原样选择
ID 2、3、10、11、13。五条分别承担正常完成、缺少注册工具、数据前置条件停止、多步故障恢复
和 Manager 澄清；这种选择保留 User-centric RS 主线，同时避免把科学代理有效性重新塞入第一章。

比较条件压缩为 B1 静态预先规划、B2 观测后调整计划、B3 在 B2 上增加逻辑检查点。只有任务
11 注入一次 operational threshold write failure；它发生在 NDVI 成功之后，不改变指数、阈值或
遥感方法。评分只检查任务闭合、计划—动作一致性、反馈后修订、记录/过程图完整性和恢复范围，
成本另报。Agent 可见输入由专用 loader 投影，fixture 和允许终态只进入外部 evaluator，并有
无泄漏测试。

本阶段状态是 `panel-ready/no-api`：49 项相关无 API 测试通过，API gate 仍关闭。五任务三条件
的一次首轮共 15 runs，只能形成 live integration smoke；若首轮配置稳定，再决定是否做三个重复
的 45-run 最小效果实验。当前不得升级 `C1-EFF-01`。

2026-08-12 完成进入 D3-light 前的三项收尾：一个共享 B1/B2/B3 runner、15 槽确定性替身决策器
dry run，以及固定但尚未获 API 授权的模型/采样/预算协议。dry run 对每个槽位实际记录计划、权限、
工具反馈、产出、终态，必要时记录修订计划、检查点和过程图；外部 evaluator 还检查完成任务的
工件类型、停止/澄清证据、故障证据和 B3 不重复 NDVI 的恢复局部性。54 项相关无 API 测试通过。

这不是 live LLM runner：替身决策器不代表模型，现有 AutoGen 角色路由也尚未接入统一 runner。
其价值只是把“实验设计”推进到可运行、可评分、可拒绝 API 调用的接口边界。下一步若研究者
